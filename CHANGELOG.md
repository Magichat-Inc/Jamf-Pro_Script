...
#### Jamf Pro 連携対応
- `sleep 60` を `sleep 5` に変更し、`shutdown -r now` をコメントアウト。再起動は Jamf Pro ポリシーの再起動オプションに委譲
...
#### バグ修正・品質改善
- `set -euo pipefail` を追加（未定義変数・コマンド失敗での即時終了）
- JSON パースを `grep | sed` から `/usr/bin/plutil` に変更（Xcode CLI Tools 未インストール環境での動作を保証しつつ、JSON 構造変化や複数マッチによる誤動作を防止）
